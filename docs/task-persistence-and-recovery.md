...
CLI 默认不执行上述流程，除非未来新增显式命令，例如：

```bash
wechat-archiver tasks status
wechat-archiver tasks retry <task_id>
wechat-archiver tasks clear --older-than 30d
```
...
当前持久化、runner 接入、只读历史查询和 retry 候选准备已实现，后续继续补显式 retry 命令：
...
- 单元测试覆盖 schema 初始化、幂等迁移、终态更新、runner/store 集成、只读历史查询、retry 候选安全边界和敏感参数不落盘。
