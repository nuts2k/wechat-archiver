...
CLI 默认不执行上述流程。当前已有显式只读命令可读取指定 app SQLite，但不会自动创建 app 配置库，也不会执行 retry：
...
当前持久化、runner 接入、只读历史查询、retry 候选准备和显式只读 CLI 已实现，后续继续补真正执行 retry 的命令：
...
- 可为已完成、失败、取消或中断任务生成 retry 候选；`queued/running`、缺少必要目录、缺少任务类型、参数摘要非对象或包含疑似明文 key/token/cookie/secret 的任务会标记为不可重试。
- CLI `tasks` 子命令必须显式传入 `--app-db`，只读打开已有 app SQLite；缺失路径不会创建数据库。
- 单元测试覆盖 schema 初始化、幂等迁移、终态更新、runner/store 集成、只读历史查询、retry 候选安全边界、CLI 只读任务命令和敏感参数不落盘。
...
- 不实现后台 daemon。
- 不实现跨进程实时事件订阅。
- 不实现真正执行任务的 retry 命令或自动执行 retry。
- 不实现任务历史导出。
- 不实现任务清理策略。
