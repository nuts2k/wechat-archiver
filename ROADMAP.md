...
- core 已提供 `TaskStore` trait 和 `SqliteTaskStore` 最小实现，可在独立 app SQLite 中记录 `task_runs`、更新进度快照、写入完成/失败/取消终态，并在启动恢复时把遗留 `queued/running` 标记为 `interrupted`。
...
- 继续完善长任务控制：让 `TaskRunner` 可选接入 `TaskStore`，补充任务历史只读查询、显式 retry、多 worker 配置、暂停/恢复和更细粒度进度节流，为未来 Tauri 做准备。
