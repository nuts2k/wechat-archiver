...
- core 暴露 `TaskStore`、`TaskListQuery` 和 `SqliteTaskStore`，`TaskRunner` 可显式接入 store 来记录和查询任务历史、进度快照和启动恢复的 `interrupted` 状态；持久化和重启恢复边界见 [docs/task-persistence-and-recovery.md](docs/task-persistence-and-recovery.md)。
